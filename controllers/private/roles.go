...
func (um *roles) PrivateRoute(ctx gctx.Context, b *builders.PrivateRouteBuilder) {
	b.NewHandler().Register(builders.GET, "/select", um.rolesSelectGet).SetRequiredPrivileges([]string{RoleSelectGet}).Build(ctx)
	b.NewHandler().Register(builders.PUT, "/user/{user}", um.setUserRolePut).SetRequiredPrivileges([]string{SetUserRolePut}).Build(ctx)
}
